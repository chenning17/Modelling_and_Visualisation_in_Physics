...
public class SIRSimmunePlot{
    ...
    static void visualize(Visualization vis, MyMatrix grid, int rows, int cols){
        for (int r = 0; r < rows; r++){
            for(int c = 0; c < cols; c++){
                ...
                vis.set(c, r, grid.getElement(r,c) == 0.0 ? Color.ORANGE : (grid.getElement(r,c) == 1.0? Color.RED : (grid.getElement(r,c) == 2.0? Color.GREEN : Color.BLACK)) );
            }
        }
        vis.draw();
    }
    ...
    static void Infect(MyMatrix data, double prob1, double prob2, double prob3){
        ...
        if(data.getElement(row, col) == 3){
            //do nothing it is immune
        }
        
        //if the random element is susceptible then infect with certain probability
        else if(data.getElement(row, col) == 0){
    ...
    public static void main (String[] args) throws Exception{
        ...
        if(args.length != 2){
            System.out.println("Incorrect number of arguments...\n1) input dimension \n2) level of detail for contour plot \n ");
            System.exit(0);
        }
        ...
        //Random.nextInt(1);   //supposed to randomly make number between 0,1
        ...
        for(int p1 = 1; p1<=detail; p1++){
            ...
            for(int pImmune = 1; pImmune<=detail; pImmune++){

                immuneFraction = (double)pImmune/(detail-1/((detail-1)*10));        //make sure probability value is in range 0-1
                ...
                for(int rep = 0; rep<=REPEATS; rep++){
                    ...
                    //initialise array to hold values and assign values representing S (0), I (1), R (2) and Immune (3)
                    for( int i=0; i<rows; i++ ) {
                        for(  int j=0; j<cols; j++ ) {
                            
                            probImmune = Math.random();
                            
                            if(probImmune<= immuneFraction){
                    ...
                    while(loops < SIMS){    
                        ...
                        if(loops>EQUILIBRIUM){
                            ...
                            if(loops%10==0){
                                ...
                                //do all variance etc calculations here.................
                    ...
                    // contour[p1][pImmune] = averageInfected/N;  // <= <I>
                    //calculate variance for this value: (<I^2> - <I>^2)/N
                    //variance[p1][pImmune] = (averageInfectedSquared - (averageInfected*averageInfected))/(N*N);
                ...
                contour[p1][pImmune] = contourSum/REPEATS;
                variance[p1][pImmune] = varianceSum/REPEATS;
        ...
        String fname = "Immune_p1-p3_ave_frac.dat";
        PrintWriter output = new PrintWriter(new FileWriter(fname));
        
        String fname2 = "Immune_p1-p3_variance.dat";
