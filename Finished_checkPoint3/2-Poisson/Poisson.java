...
public class Poisson{
    ...
    static void updatePhi(MyMatrix3D phi, MyMatrix3D rho, double dx){
        ...
        double[][][] temp = new double[phi.rows()][phi.cols()][phi.height()];
            ...
            for(int j = 0; j<phi.cols(); j++){
                for(int k = 0; k<phi.height(); k++){
                    ...
                    if(i == 0 || j == 0 || k == 0 || i%(rho.rows()-1) == 0 || j%(rho.cols()-1) == 0 || k%(rho.height()-1) == 0){
                        ...
                        temp[i][j][k] = (1.0/6.0)* (i_plus+ i_minus + j_plus + j_minus + k_plus + k_minus  + dx*dx*rijk);
                    }
                }
            }
        }
        ...
        for(int i = 0; i<phi.rows(); i++){
            for(int j = 0; j<phi.cols(); j++){
                for(int k = 0; k<phi.height(); k++){
                    ...
                    phi.setElement(i,j,k,  temp[i][j][k]);
                }
            }
        }
    }
    ...
    static void  toFileE(MyMatrix3D Ex, MyMatrix3D Ey, MyMatrix3D Ez) throws Exception{
        ...
        String fname = "E_field_data_P.dat";
        PrintWriter output = new PrintWriter(new FileWriter(fname));
        
        String fname2 = "E_field_vs_centre_P.dat";
        PrintWriter output2 = new PrintWriter(new FileWriter(fname2));
        
         String fname3 = "E_field_slice_P.dat";
        PrintWriter output3 = new PrintWriter(new FileWriter(fname3));
        ...
        for(int i = 0 ; i<Ex.rows(); i++){
            for(int j = 0 ; j<Ex.cols(); j++){
                    ...
                    output3.printf("%d %d %f %f \n", i, j, Ex.getElement(i,j,Ex.height()/2), Ey.getElement(i,j,Ex.height()/2));
                ...
                for(int k = 0 ; k<Ex.height(); k++){
                    ...
                    if(i>=Ez.rows()/2 && j == Ez.cols()/2 && k == Ez.height()/2){
                        //output the z component of the e field vs distance from centre to a file
    ...
    public static void main(String[] args)  throws Exception{
        ...
        String fname = "Potential_data_P.dat";
        PrintWriter output = new PrintWriter(new FileWriter(fname));
        ...
        //write data from the centre slice to file so (3D) contour plots can be made
        String fname2 = "Potential_vs_centre_P.dat";
        PrintWriter output2 = new PrintWriter(new FileWriter(fname2));
